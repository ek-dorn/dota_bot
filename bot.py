...
async def keyword_response(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for keyword in keywords:
        if keyword in message:
            await update.message.reply_text(f"Заходят как−то в таверну {mentions}. А бармен у них и спрашивает: «А мидера у вас тоже два?»", parse_mode=ParseMode.MARKDOWN_V2)
            return
